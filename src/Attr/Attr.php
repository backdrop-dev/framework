...
class Attr implements Attributes {
	...
	public function all(): array {
		...
		$defaults = apply_filters( "backdrop/attr/{$this->name}/defaults", $defaults, $this->context, $this );

		// Merge the attributes with the defaults.
		$this->attr = wp_parse_args( $this->input, $defaults );

		// Apply filters to the parsed attributes.
		$this->attr = apply_filters( 'backdrop/attr', $this->attr, $this->name, $this->context );
		$this->attr = apply_filters( "backdrop/attr/{$this->name}", $this->attr, $this->context );
		...
		$hook = "backdrop/attr/{$this->name}/class";
